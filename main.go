...
func main() {
	...
	*/
	...
	myStorage, err := storage.FabricaProducto(driver)
	if err != nil {
		log.Fatalf("Fabrica Producto Error: %v", err)
	}

	servicioProducto := producto.NewServicio(myStorage)

	m, err := servicioProducto.GetAll()
	if err != nil {
